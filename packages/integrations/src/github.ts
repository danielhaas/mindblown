...
import { ForgeApiError, type ForgeClient, type IssueCloseEvent, type UpdateIssueInput } from './forge/types.js';
import { githubFetch, paginateGitHub } from './forge/pagination.js';

export type { IssueCloseEvent } from './forge/types.js';
...
// Pagination, its error classes and the Link-header walker live in
// `./forge/pagination.ts` since #368 (the Gitea client uses them too);
...
function buildExternalLink(
  ...
  return {
    // The link names its forge (`github` | `gitea`); readers test with
    // `isForgeLink` from @mindblown/core rather than comparing to 'github'.
    provider: forge.endpoint.kind,
...
export async function findClosingPrsForIssue(
  ...
  // The forge reads the issue's timeline to its END (ascending) and hands
  // back the same-repo PRs that reference it — a mention is enough here,
  // the closing keyword is re-checked on the PR itself below.
  const seen = await forge.listCrossReferencingPullRequests(repoOwner, repoName, issueNumber);
...
export async function getIssueCloseEvent(
  ...
  return forge.getLatestCloseEvent(repoOwner, repoName, issueNumber);
}
...
export async function createGitHubIssue(
  ...
  return {
    issue,
    externalLink: buildExternalLink(repoOwner, repoName, issue, forge),
  };
}
...
export async function updateGitHubIssue(
  ...
  const patchBody: UpdateIssueInput = {
    title: node.text,
    body,
    labels,
  };
  ...
  const updatedIssue = await forge.updateIssue(owner, repo, issueNumber, patchBody);
...
export async function reopenGitHubIssue(
  ...
  return forge.updateIssue(owner, repo, issueNumber, { state: 'open', state_reason: 'reopened' });
}
...
export async function closeGitHubIssue(
  ...
  return forge.updateIssue(owner, repo, issueNumber, { state: 'closed', state_reason: reason });
}
...
export async function setGitHubIssueMilestone(
  ...
  const milestones = await forge.listMilestones(repoOwner, repoName);
  ...
  await forge.updateIssue(repoOwner, repoName, issueNumber, { milestone: m.ref });
  if (removeLabel) {
    const res = await forge.removeIssueLabel(repoOwner, repoName, issueNumber, removeLabel);
    // 404 = the label was not on the issue; nothing to remove.
    if (res.status !== 404 && (res.status < 200 || res.status >= 300)) {
      throw new ForgeApiError(res.status, res.bodyText, forge.endpoint.kind);
    }
  }
  return { milestoneNumber: m.ref };
}
...
export async function importGitHubIssues(
  ...
  await paginateGitHub<GitHubIssue>(
    forge.issuesListPath(repoOwner, repoName, { state, perPage, sort: 'created', direction: 'asc' }),
    forge,
    {
      maxPages: Math.ceil(MAX_FETCHED_ISSUES / perPage),
      onPage: (batch) => {
        // Filter out pull requests (GitHub API returns PRs in issues endpoint)
        issues.push(...batch.map((i) => forge.normalizeIssue(i)).filter((i) => !i.pull_request));
      },
      onTruncated: () => {
        console.warn(
...
export async function fetchChangedIssues(
  ...
  const { truncated } = await paginateGitHub<GitHubIssue>(
    forge.issuesListPath(repoOwner, repoName, {
      state: 'all',
      perPage,
      sort: 'updated',
      direction: 'asc',
